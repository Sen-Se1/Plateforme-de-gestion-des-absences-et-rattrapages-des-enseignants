...
import { getMyTimetableAsTeacher } from "@/lib/api/emploisDuTemps";
...
export default function TeacherTimetablePage() {
  ...
  const loadTimetable = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await getMyTimetableAsTeacher(1, 100);
  ...
  return (
    <div className="space-y-8">
      ...
      <WeeklyTimetable
        courses={courses}
        title="Planning de Cours - Enseignant"
        subtitle="Votre grille de cours hebdomadaire validée"
      />
    </div>
  );
}
